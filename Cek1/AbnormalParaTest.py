...
def train(data, model,optim, logger,epoch,logdir):
    ...
    bpp=JpegCompress(img=datahat,quality=25)

    writer.add_scalar('scalar/bpp', bpp, (epoch))
    writer.add_scalar('scalar/Rate', Rate.item(), (epoch))
    writer.add_scalar('scalar/Distortion', distortion.item(), (epoch))
...
if __name__ == '__main__':
    ...
    training_config = OutputConfig(logdir=os.path.join('/output','logs'),
                                   ckptdir=os.path.join('/data/wym123/paradata','diffjpeg_sin_cek1'))
    ...
    for epoch in range(0, args.epoch):
        train(data=img,model=net,optim=optimizer,logger=logger,epoch=epoch,logdir=training_config.logdir)
        if epoch%5==0:
